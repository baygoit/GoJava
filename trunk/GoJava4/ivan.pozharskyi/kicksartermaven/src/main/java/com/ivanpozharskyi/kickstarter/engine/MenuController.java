...
public class MenuController {
	...
	private void runMenuProject() throws SQLException {
		currentProject = projects.getProjectsInCategory(currentCategory.getId())
				.get(choice-1);												//TO DO magic number
		menuRunner.setMenu(new MenuProject(projects
				.getProjectsInCategory(currentCategory.getId()), printer));
	...
	private void runMenuProjects() throws SQLException {
		...
		menuRunner.showMenu();
		choice = reader.read();
		if(choice < 0 || choice > projects.getProjectsInCategory(currentCategory.getId()).size()){
