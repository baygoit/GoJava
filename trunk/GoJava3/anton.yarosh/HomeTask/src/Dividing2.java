...
public class Dividing2 {
	...
	public static void main(String[] args) {
		...
		String outAfter="\n ";
		String outFinal="";
		int lengthA=input.split("/")[0].length();
		int lengthB=input.split("/")[1].length();
		int gap=lengthA;
		int poz=1;
		int a=Integer.parseInt(input.split("/")[0]);
		int b=Integer.parseInt(input.split("/")[1]);
		int tmp=(int)((a/Math.pow(10, lengthA-1))/(b/Math.pow(10, lengthB-1)));
		...
		if(i==0)
			for (int j=1; j<=lengthA-new String(b*rez+"").length()+1; j++)
			{outBefore+=" ";}
		else
			for (int j=1; j<=lengthA-i-new String(b*rez+"").length()+1; j++)
			{outBefore+=" ";}
		...
		{
			ost=(int) (ost*Math.pow(10, 1)+(a%Math.pow(10, i))/Math.pow(10, i-1));
			for(int j=1; j<=poz; j++)
				outAfter+=" ";
			outAfter+="_"+ost+"\n";
			...
			{i--;}
			rez=(ost/b);
			for (int j=1; j<=poz+1+new String(ost+"").length()-new String(rez*b+"").length();j++)
				outAfter+=" ";
			outAfter+=rez*b+"\n";
			lastOst=ost;
			poz++;
			...
			for (int j=1; j<=new String(ost+"").length();j++)
				outAfter+="-";
			outAfter+="\n";
			ost=ost%b;
		...
		if(ost==0)
		{
			for(int j=1; j<=poz+new String(lastOst+"").length()-1; j++)
				outAfter+=" ";
			outAfter+=ost+"\n";
		}
