...
public class ProjectDB {
	...
	public List<Project> getProjectByCategoriId(int categoryId){
		Connection c = null;
		Statement st = null;
		Project proj = null;
		...
		try{
			...
			String string = "SELECT * FROM projects WHERE id_category ='" + categoryId + "';";
	...
	public void createProject(Project project) {
		...
		try{
			...
			String string = "INSERT INTO projects (id,name,description,need_money,have_money,"
						+ "end_project,project_history,demo_link,id_category)" + "VALUE(" + newId 
							 + project.getName() + project.getDescription() + project.getNeedMoney() +
						project.getHaveMoney() + project.getDaysBeforeEnd() + project.getProjectHistory() +
	...
	public void updateProjectHaveMoney(Project project) {
		...
		try{
			c = DriverManager.getConnection("jdbc:postgresql://localhost:5432/kickstarterdb",
					"postgres", "12345");
			st = c.createStatement();
			String string = "UPDATE projects SET have_money =" + haveMoney + " where name =" +
							project.getName() + ";";
			st.executeUpdate(string);
			c.commit();
