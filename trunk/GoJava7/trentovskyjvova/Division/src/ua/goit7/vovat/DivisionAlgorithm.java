...
public class DivisionAlgorithm {
	...
	private static void division(int dividend, int divisor, String leftMargin,
			QuotientBuilder quotientBuilder, ArrayList<String> result) {

		int curentDividend = getCurentDividend(Integer.toString(dividend), 
				Integer.toString(divisor), quotientBuilder);
		...
		if (checkEnd(dividend, quotientBuilder, curentDividend, product)) {
			...
			int degree = Math.max(length(dividend) 
					- Math.min(length(curentDividend), length(product)), 0);
			int difference = (int) Math.pow(BASE_MULTIPLIER, degree);
			
			if (dividend == (product * difference)) {
				quotientBuilder.multiply(difference);
			} else {			
				int nextStepDividend = findNextStepDividend(dividend, divisor, 
						quotientBuilder, curentDividend, product, difference);

				division(nextStepDividend, divisor, 
						leftMargin, quotientBuilder, result);
			}
		}

	}

	private static boolean checkEnd(int dividend, 
			QuotientBuilder quotientBuilder, int curentDividend, int product) {
		
		return (product % dividend != 0 || curentDividend % product != 0)
				&& (quotientBuilder.get().length() < DECIMAL_PLACES);
	}

	private static int findNextStepDividend(int dividend, int divisor, 
			QuotientBuilder quotientBuilder, int curentDividend, 
			int product, int difference) {
	...
	private static int divide(int curentDividend, int divisor, 
			QuotientBuilder quotientBuilder) {
	...
	private static Integer getCurentDividend(String dividend, String divisor,
			QuotientBuilder quotientBuilder) {

		int substring = Integer.parseInt(
				dividend.substring(0, Math.min(dividend.length(), divisor.length())));
