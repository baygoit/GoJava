...
public class Division {
	...
	public void getPartDivident(){
		...
		if (dividentPartStartPos == 0){
			dividentPart=Integer.parseInt(divident.substring(dividentPartStartPos,dividentPartEndPos));
		}
		if (dividentPart < divisorInt && dividentPartStartPos == 0 && dividentPartStartPos<divisor.length()) {
    		dividentPartEndPos++;
    		dividentPart=Integer.parseInt(divident.substring(dividentPartStartPos,dividentPartEndPos));
    	} else if(dividentPartStartPos > 0 && dividentPartStartPos<divident.length()){
    		dividentPart=Integer.parseInt(Integer.toString(remainder)+divident.substring(dividentPartStartPos,dividentPartEndPos));
